...
4. **Differentiable Structural Loss Fine-Tuning (Concluded — Negative Result):** RLAIF using differentiable NOTEARS penalties resulted in mode collapse (the model erases edges to trivially satisfy structural constraints). The winning approach is decode-time deterministic repair via the Constraint Solver, which achieves 10% SVR with no training. See `docs/02_EXPERIMENTS_AND_RESULTS.md` for the full ablation arc and `PROJECT_STATUS.md` for current state.
