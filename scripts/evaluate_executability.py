...
def main():
    ...
    print(f"Evaluating Executability on {device}...")
    ...
    # Generate large batch to find perfect graphs
    dataloader = DataLoader(dataset, batch_size=64, shuffle=True)
    
    num_batches_to_eval = 8
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            # evaluate_batch doesn't return per-graph results, so we have to re-evaluate locally to filter
            for b in range(B):
            ...
            print(f"Batch {i+1}/{num_batches_to_eval} - Found {perfect_graphs_found} perfect graphs so far...")

    print("\n" + "="*50)
    print("EXECUTABILITY AUDIT RESULTS")
    print("="*50)
    print(f"Total structurally perfect graphs evaluated: {perfect_graphs_found}")
