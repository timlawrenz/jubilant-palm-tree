...
- **[CONCLUDED — PASS] Exp 2 — Build the A→C Conditioning Path (the Legislative Branch)** — `A→C demonstrated end-to-end`
  ...
  - *Result:* **PASS, reproduced.** N=512 held-out, LLM→BoM→AR e2e typed-F1: primary 0.4901, repro 0.4955 (temp 0.3). Gate 0.20 → 2.45×; Δ+0.23 over floor (0.260); ceiling 0.776 validated (harness intact); no-leak guardrail held; BoM fidelity 0.40 (AR tolerates imperfect BoMs). A→C (LLM intent → executable graph) demonstrated end-to-end on real held-out methods.
  - *Why it matters:* Tests the founding premise (intent → executable structure) end to end. **The Legislative → AR Executive → Judicial chain is complete and functional.** Next: MQ3 goal gate (benchmark vs LLM code gen, pre-registered `e923783`).
